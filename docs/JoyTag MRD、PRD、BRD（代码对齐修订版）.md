...
**中文锚点流程**：淘宝搜索建议采集中文长尾词，使用固定 bootstrap 种子与最近锚点/高热度建议词组成动态种子池；每轮最多查询 80 个种子（动态最多 64、固定最多 16）。系统解析淘宝返回的第二字段，在单次响应内以 `log1p` 归一化并与位置分合成相对热度，再按规范化词去重、新颖度和热度排序，结合来源快照、候选观察和进度文件跳过近期已处理词，随后用本地 Embedding 批量写入 `cn_anchors`。该热度不是绝对搜索量，中文锚点主要承担跨语言对齐的底座作用，不在当前流程中虚构六国语言逐词合规评估结果。
